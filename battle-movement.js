...
(function () {
  ...
  function updateMoveButton() {
    ...
    const usable = unit && unit.side === state.playerSide && unit.hp > 0 &&
      state.phase === "PLANNING" && !state.turnSpent && !state.movement.moved[unit.id];
  ...
  function toggleMoveMode() {
    ...
    if (!unit || unit.side !== state.playerSide || unit.hp <= 0) {
      showToast("Select one of your units first");
      return;
    }
    if (state.phase !== "PLANNING" || state.turnSpent) {
      showToast("Movement is not available now");
      return;
    }
    if (state.movement.moved[unit.id]) {
      showToast("This unit has already moved this turn");
      return;
    }
  ...
  function occupied(point, unit) {
    return state.units.some(other =>
      other.id !== unit.id && other.hp > 0 && distance(point, other) < 6
    );
  }
  ...
  function moveUnitTo(point) {
    ...
    if (d > range) {
      showToast(`OUT OF MOVE RANGE — ${range.toFixed(1)} max`);
      return;
    }

    if (occupied(point, unit)) {
      showToast("POSITION OCCUPIED");
      return;
    }

    // Keep units in their broad battlefield lane.
    if (unit.type !== "radar" && point.y < 50) {
      showToast("This unit must remain in the sea lane");
      return;
    }
    ...
    // Moving slightly reduces engine condition, making repeated maneuvering meaningful.
  ...
  function install() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      setTimeout(function () {
        ...
        // Registered after the original battlefield handler so movement gets first priority when active.
        ...
        document.addEventListener("click", function () {
          setTimeout(updateMoveButton, 20);
        });
